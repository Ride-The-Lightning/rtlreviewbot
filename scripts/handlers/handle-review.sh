...
# ---------------------------------------------------------------------------
# Step 5+6 — invoke the /code-review skill and parse its output.
#
# Auth fallback (api → oauth) and the parser-as-oracle success criterion
# both live in scripts/lib/invoke-claude.sh; this handler just composes
# the prompt and calls the lib's invoke_claude_for_review.
#
...
# scripts/lib/invoke-claude.sh handles auth fallback (api → oauth) plus
# the parser-as-oracle success criterion: an attempt only counts as
# success when both the claude CLI exits 0 AND parse-review-output.sh
# accepts the output. Soft failures (e.g. "Credit balance is too low"
# printed to stdout with exit 0) trigger fallback to the next attempt.
